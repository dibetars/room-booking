...
| 22 Jun | Admin Dashboard — Channel Badges, Booking Detail & CSV Export | 2.0 | Added OTA channel identification to admin bookings table using Beds24 `referer` field — normalised to display badges (Booking.com, Airbnb, Hostelworld, Expedia, Direct). Implemented click-through booking detail modal showing full guest info, stay details, payment status, and inline confirm/cancel/mark-paid actions. Added CSV export of the current visible bookings view with all fields including channel and payment reference. | None. |
| 23 Jun | Revenue & Analytics Module | 2.5 | Built `GET /api/admin/analytics` aggregating 180-day booking history from Beds24 into monthly revenue, channel breakdown, and room performance data server-side. Built Revenue page with 4 stat cards, horizontal monthly bar chart, channel share table, and avg length-of-stay breakdown by month. | None. |
| 24 Jun | Room Performance Module & Room Editing | 2.5 | Built Room Performance page showing bookings, revenue, nights occupied, and avg stay per room with revenue share bars. Added room edit modal backed by a new Supabase `room_overrides` table allowing name, description, rack rate, and max occupancy to be updated at runtime without a redeploy. Graceful fallback to static config if table not yet created. | Required new Supabase table (`room_overrides`) — SQL migration documented in route file. |
| 25 Jun | Admin Sidebar Layout & Bookings Lookback Selector | 4.0 | Replaced admin top navigation bar with a standard left sidebar layout — dark green brand sidebar with icons and active indicator dot, shared across all admin pages via a Next.js `layout.tsx` so individual pages only render their content. Sidebar carries Bookings, Messages, Revenue, Rooms nav tabs with New Booking and Logout pinned at the bottom. Extended admin bookings with a 30d / 90d / 180d / 1y period selector (default raised from 7 to 30 days) — switching period re-fetches automatically. Stubbed Beds24 messaging tab as coming soon. | Sidebar layout required removing `AdminNav` from every page and updating each page's root wrapper to integrate cleanly with the shared layout component. |
| 26 Jun | New Booking Page Redesign & API Caching Layer | 3.0 | Redesigned the New Booking admin form into a two-column layout: stay details and guest fields left, live booking summary card (room, nights, auto-calculated total) with pricing override, notes, and action buttons right. Added server-side in-memory caching to all three admin API routes (bookings 2-min TTL, analytics 5-min TTL, rooms 10-min TTL) with write-through invalidation on booking creation and room edits so navigating between admin sections is instant on repeat visits. | None. |
