    ...
    <main className="min-h-screen bg-[#f5f0e8] px-4 py-8">
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow p-5 space-y-2">
          ...
          <p className="text-sm text-gray-500">{adults} adult{Number(adults) !== 1 ? 's' : ''}{Number(children) > 0 ? `, ${children} child${Number(children) !== 1 ? 'ren' : ''}` : ''}</p>
          <div className="border-t pt-2 flex justify-between">
            <span className="text-sm text-gray-500">GHS {perNight.toLocaleString()} × {nights} nights</span>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow p-5 space-y-4">
          ...
          >
            {submitting ? 'Processing…' : `Pay GHS ${totalPriceGHS.toLocaleString()}`}
          </button>
